...
def analyze_pcap(pcap_file):
    ...
    pcap_file_new = 'new_pcap_file.pcap'
    ...
    try:
        ...
        num_messages_with_attacks_rs2, num_attacks_instances_rs2 = count_packets(snort_pcap_file_rs2)
        ...
        # Elimina los archivos generados por Snort para RS2
        snort_files_rs2 = glob.glob('snort.log.*')
        for file in snort_files_rs2:
            os.remove(file)
        ...
        num_messages_with_attacks_opt, num_attacks_instances_opt = count_packets(snort_pcap_file_opt)
        ...
        u2boat_cmd = f"u2boat -t pcap unified2.log.* {pcap_file_new}"
        ...
        num_messages_with_attacks_rs1, num_attacks_instances_rs1 = count_packets(pcap_file_new)

        # Contar flujos únicos en el nuevo pcap generado
        num_flows_with_attacks_rs1 = count_flows(pcap_file_new)

        # Ejecuta tranalyzer en el nuevo pcap generado
        tranalyzer_cmd_rs1 = f"tranalyzer -r {pcap_file_new}"
        ...
        # Verifica si el archivo de resumen existe
        summary_file_rs1 = os.path.splitext(pcap_file_new)[0] + '_flows.txt'
        if not os.path.isfile(summary_file_rs1):
            print(f"El archivo de resumen {summary_file_rs1} no existe.")
            return
        
        # Convierte el archivo de resumen de tranalyzer a CSV
        csv_file_rs1 = os.path.splitext(pcap_file_new)[0] + '_flows.csv'
        ...
        # Contar paquetes únicos y totales en el nuevo pcap generado
        num_messages_with_attacks_rs3, num_attacks_instances_rs3 = count_packets(pcap_file_new)

        # Contar flujos únicos en el nuevo pcap generado
        num_flows_with_attacks_rs3 = count_flows(pcap_file_new)

        # Ejecuta tranalyzer en el nuevo pcap generado
        tranalyzer_cmd_rs3 = f"tranalyzer -r {pcap_file_new}"
        ...
        # Verifica si el archivo de resumen existe
        summary_file_rs3 = os.path.splitext(pcap_file_new)[0] + '_flows.txt'
        if not os.path.isfile(summary_file_rs3):
            print(f"El archivo de resumen {summary_file_rs3} no existe.")
            return
        
        # Convierte el archivo de resumen de tranalyzer a CSV
        csv_file_rs3 = os.path.splitext(pcap_file_new)[0] + '_flows.csv'
